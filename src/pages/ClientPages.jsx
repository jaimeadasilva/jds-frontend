...
import {
  ...
  Empty, CompletionBanner, Btn, Input, Textarea, SectionHeader, Select,
  goalColor
} from "../components/UI";
import { WeightGraph } from "../components/WeightGraph";
import { ClientWorkoutTab } from "./WorkoutTab";
import { Modal } from "../components/Modal";
import { displayWeight, displayHeight, calcBMI, displayIBW, bmiCategory, kgToLbs, lbsToKg, calcIBW } from "../utils/units";
...
export function ClientHome() {
  ...
  const submitWeight = async () => {
    ...
    try {
      ...
      await refreshUser(); // update stats immediately
  ...
  // Stats — read from merged user object (now has weight_kg, height_cm from clientProfile)
  ...
  return (
    <div className="page">
      {/* Hero — no gap: no Spinner here, stats show instantly */}
      ...
      <div style={{ padding:"16px 20px" }}>
        {/* Stats row — always visible, no loading gate */}
...
// ─── Client Nutrition — weekly plans + custom meal logging ────────────────────
export function ClientNutrition() {
  const { user, token, loading: authLoading } = useAuth();
  const { toast } = useToast();
  const [nutrition, setNutrition] = useState(null);
  const [loading,   setLoading]   = useState(true);
  const [view,      setView]      = useState("plan");   // "plan" | "log"
  const [selectedDay, setSelectedDay] = useState(new Date().toISOString().slice(0,10));
  const [addModal,  setAddModal]  = useState(false);

  // Daily log stored in localStorage per date
  const getDayLog  = (date) => { try { return JSON.parse(localStorage.getItem(`meal_log_${date}`)||"{}"); } catch { return {}; } };
  const [dayLog, setDayLog] = useState(() => getDayLog(selectedDay));
  const [customEntries, setCustomEntries] = useState(() => { try { return JSON.parse(localStorage.getItem(`meal_custom_${selectedDay}`)||"[]"); } catch { return []; } });
  ...
  useEffect(() => {
    setDayLog(getDayLog(selectedDay));
    try { setCustomEntries(JSON.parse(localStorage.getItem(`meal_custom_${selectedDay}`)||"[]")); } catch { setCustomEntries([]); }
  }, [selectedDay]);

  const toggleMeal = (mealId) => {
    const updated = { ...dayLog, [mealId]: !dayLog[mealId] };
    setDayLog(updated);
    localStorage.setItem(`meal_log_${selectedDay}`, JSON.stringify(updated));
    if (!dayLog[mealId]) toast.success("Meal logged ✓");
  };

  const addCustomEntry = () => {
    if (!newEntry.name.trim()) return;
    const entry = { id:Date.now(), ...newEntry, calories:+newEntry.calories||0, protein:+newEntry.protein||0, carbs:+newEntry.carbs||0, fats:+newEntry.fats||0 };
    const updated = [...customEntries, entry];
    setCustomEntries(updated);
    localStorage.setItem(`meal_custom_${selectedDay}`, JSON.stringify(updated));
  ...
  const n = nutrition;
  const loggedMeals   = n?.meals?.filter(m => dayLog[m.id]) || [];
  const loggedCal     = loggedMeals.reduce((s,m)=>s+m.calories,0) + customEntries.reduce((s,e)=>s+e.calories,0);
  const loggedProtein = loggedMeals.reduce((s,m)=>s+m.protein_g,0) + customEntries.reduce((s,e)=>s+e.protein,0);
  const loggedCarbs   = loggedMeals.reduce((s,m)=>s+m.carbs_g,0)   + customEntries.reduce((s,e)=>s+e.carbs,0);
  const loggedFats    = loggedMeals.reduce((s,m)=>s+m.fats_g,0)    + customEntries.reduce((s,e)=>s+e.fats,0);

  // Generate day options (today + 6 days back)
    ...
    <div className="page">
        ...
        {loading ? <Spinner /> : !n ? (
          ...
          <>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                <div>
                  ...
                  <div style={{ fontSize:12, color:loggedCal>n.calories?"var(--rose)":"var(--muted)", marginTop:2 }}>
                    {loggedCal>n.calories ? `${loggedCal-n.calories} kcal over target` : `${n.calories-loggedCal} kcal remaining`}
                  </div>
                </div>
              </div>
              <ProgressBar value={(loggedCal/n.calories)*100} color={loggedCal>n.calories?"var(--rose)":"var(--royal)"} height={7} />
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginTop:12 }}>
                {[{l:"Protein",v:loggedProtein,t:n.protein_g,c:"var(--royal)"},{l:"Carbs",v:loggedCarbs,t:n.carbs_g,c:"var(--amber)"},{l:"Fats",v:loggedFats,t:n.fats_g,c:"#8B5CF6"}].map(m=>(
                  <div key={m.l} style={{ background:"var(--bg)", borderRadius:9, padding:"8px 9px" }}>
                    <div style={{ fontSize:13, fontWeight:800, color:m.c, fontFamily:"var(--font-display)" }}>{m.v}g <span style={{ fontSize:10, fontWeight:500, color:"var(--muted)" }}>/ {m.t}g</span></div>
            ...
            {n.meals?.map(meal => {
              ...
              return (
                <Card key={meal.id} style={{ marginBottom:10, opacity:done?0.6:1, cursor:"pointer", transition:"opacity 0.25s" }} onClick={()=>toggleMeal(meal.id)}>
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                      <div>
                        ...
                        <div style={{ display:"flex", flexWrap:"wrap", gap:5, marginTop:7 }}>
                          ...
                          {entry.protein>0&&<Badge label={`P ${entry.protein}g`} color="var(--royal)" />}
                          {entry.carbs>0&&<Badge label={`C ${entry.carbs}g`} color="var(--emerald)" />}
                          {entry.fats>0&&<Badge label={`F ${entry.fats}g`} color="#8B5CF6" />}
                        </div>
                      </div>
                      <button onClick={()=>removeCustomEntry(entry.id)} style={{ background:"none", border:"none", cursor:"pointer", fontSize:16, color:"var(--muted2)", padding:4 }}
      ...
      <Modal open={addModal} onClose={()=>setAddModal(false)} title="Log Food or Meal">
        <p style={{ fontSize:13, color:"var(--muted)", marginBottom:14, lineHeight:1.6 }}>
          Add anything you ate that wasn't in your plan — extra meals, snacks, substitutions.
        </p>
        ...
        <Textarea label="Notes (optional)" value={newEntry.notes} onChange={e=>setNewEntry(p=>({...p,notes:e.target.value}))} placeholder="e.g. Replaced lunch, had a cheat meal" rows={2} />
        <div style={{ display:"flex", gap:10 }}>
          <Btn variant="secondary" onClick={()=>setAddModal(false)} full>Cancel</Btn>
          <Btn variant="primary" onClick={addCustomEntry} full>Add Entry</Btn>
        </div>
      </Modal>
...
export function ClientMedical() {
  ...
  useEffect(() => {
    if (authLoading || !user?.id || !token) return;
    medicalAPI.list(user.id, token).then(d=>setMedical(Array.isArray(d)?d:d?.data||[])).catch(()=>{}).finally(()=>setLoading(false));
  }, [user?.id, token, authLoading]);
          ...
          <>
            <div style={{ background:"var(--amber-pale)", borderRadius:"var(--radius)", border:"1px solid #FCD34D30", padding:"12px 16px", marginBottom:16 }}>
              <p style={{ margin:0, fontSize:13, color:"#92400E", fontWeight:600, lineHeight:1.6 }}>
                ⚠️ These notes are important for your training safety. Please read them carefully and follow your coach's guidance.
              </p>
            </div>
            {medical.map(m=>{
...
export function ClientProfile() {
  ...
  const changeLbs = changeKg!==null ? (kgToLbs(Math.abs(changeKg))) : null;
  ...
  // Convert weight history to display in lbs for graph
            ...
            <Card style={{ marginBottom:16 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:14 }}>
                <div>
                  ...
                  {latest&&<div style={{ fontSize:26, fontWeight:800, color:"var(--text)", fontFamily:"var(--font-display)", letterSpacing:"-0.03em", marginTop:4 }}>{kgToLbs(latest.weight_kg)} <span style={{ fontSize:14, fontWeight:500, color:"var(--muted)" }}>lbs</span></div>}
                </div>
                {changeLbs!==null&&(
              ...
              <>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                  {[...sorted].reverse().slice(0,15).map((entry,i,arr)=>{
                    ...
                    const diffLbs=diffKg!==null?kgToLbs(Math.abs(diffKg)):null;
                    return (
        ...
        {tab==="equipment"&&(
          <div>
            {equipment.length===0
              ...
              : (<>
                  <p style={{ fontSize:13, color:"var(--muted)", marginBottom:16, lineHeight:1.6 }}>Your workouts are tailored to this equipment.</p>
