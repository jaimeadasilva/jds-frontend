...
export function ClientHome() {
  ...
  return (
    <div className="page">
      {/* Hero */}
      <div style={{ background:`linear-gradient(145deg, #1E40AF 0%, var(--royal) 100%)`, padding:"52px 24px 28px", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ padding:"20px" }}>
        {loading ? <Spinner /> : (
          <>
            {/* Today's workout */}
            {today && (
              <Card style={{ padding:18, marginBottom:16, borderTop:"3px solid var(--royal)" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
                  ...
                  <Badge label={`${todayDone.length}/${today.exercises?.length||0}`} color={todayDone.length===today.exercises?.length && today.exercises?.length>0?"var(--emerald)":"var(--royal)"} />
                </div>
                <ProgressBar value={today.exercises?.length ? (todayDone.length/today.exercises.length)*100 : 0} color="var(--royal)" height={8} />
            ...
            {/* Calories */}
            {n && (
              <Card style={{ padding:18, marginBottom:16, borderTop:"3px solid var(--amber)" }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8 }}>
                  {[{label:"Protein",v:n.protein_g,c:"var(--royal)"},{label:"Carbs",v:n.carbs_g,c:"var(--amber)"},{label:"Fats",v:n.fats_g,c:"var(--royal-pale2)"}].map(m => (
            ...
            <Card key={day.id} style={{ marginBottom:12, overflow:"hidden", padding:0 }}>
              <button onClick={() => setExpanded(isOpen?-1:dayIdx)}
                ...
                <div style={{ display:"flex", alignItems:"center", gap:10, textAlign:"left" }}>
                  <div style={{ width:36, height:36, borderRadius:11, background: done===total&&total>0?"var(--emerald)20":"var(--royal-pale)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, fontWeight:900, color: done===total&&total>0?"var(--emerald)":"var(--royal)" }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                  <div style={{ textAlign:"right" }}>
                    <div style={{ fontSize:11, color: done===total&&total>0?"var(--emerald)":"var(--muted)", fontWeight:700 }}>{done}/{total}</div>
                    <ProgressBar value={total?(done/total)*100:0} color={done===total&&total>0?"var(--emerald)":"var(--royal)"} height={4} />
                  </div>
                  <span style={{ color:"var(--royal)", fontSize:18 }}>{isOpen?"⌃":"⌄"}</span>
                </div>
              </button>
                    ...
                    return (
                      <div key={ex.id} style={{ display:"flex", alignItems:"flex-start", gap:12, padding:"12px 0", borderBottom:"1px solid var(--line)", opacity: done?0.5:1, transition:"opacity 0.2s" }}>
                        <button onClick={() => toggleDone(ex.id)}
                          style={{ width:26, height:26, borderRadius:8, flexShrink:0, marginTop:1, background: done?"var(--royal)":"transparent", border:`2px solid ${done?"var(--royal)":"var(--line)"}`, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", transition:"all 0.15s" }}>
                        ...
                        <div style={{ flex:1 }}>
                          ...
                          {ex.video_url && <a href={ex.video_url} target="_blank" rel="noreferrer" style={{ display:"inline-flex", alignItems:"center", gap:5, marginTop:6, background:"var(--amber)", color:"#fff", borderRadius:8, padding:"5px 12px", fontSize:12, fontWeight:700, textDecoration:"none" }}>▶ Watch Demo</a>}
                        </div>
                      </div>
                    );
                  })}
          ...
          <>
            ...
            <Card style={{ padding:"20px 16px", marginBottom:16 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, justifyItems:"center" }}>
                ...
                <MacroRing label="Fats"    value={n.fats_g}    color="var(--royal-pale2)" />
              </div>
            </Card>
    ...
    <div className="page">
      ...
      <div style={{ padding:"0 20px 24px" }}>
        ...
        <button onClick={logout}
          style={{ width:"100%", marginTop:20, background:"#FEF0F0", color:"var(--rose)", border:"1px solid #FCCFCF", borderRadius:14, padding:14, cursor:"pointer", fontWeight:700, fontSize:15, fontFamily:"var(--font-body)" }}>
          Sign Out
        </button>
      </div>
      <BottomNav tabs={CLIENT_TABS} />
