...
export default function CoachHome() {
  ...
  const avgProgress = clients.length
    ? Math.round(clients.reduce((s,c) => s+(c.progress_pct||0), 0) / clients.length) : 0;

  return (
    <div className="page">
      {/* Hero */}
      <div style={{ background:"linear-gradient(150deg, #1E3A8A 0%, #2563EB 55%, #3B82F6 100%)", padding:"56px 24px 32px", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ position:"relative", zIndex:1 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginTop:22 }}>
            {[
              ...
              { label:"Completions",   value:activity.length,   icon:"✅" },
            ].map(s => (
      ...
      <div style={{ padding:"22px 20px" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:28 }}>
          <button onClick={() => navigate("/coach/clients/new")}
            ...
            <span style={{ fontSize:20 }}>➕</span>Add Client
          </button>
